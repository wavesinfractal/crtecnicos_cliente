...
class Registro extends Component {
  ...
  ActualizarState = async e  => {
   
    const { name, value } = e.target;
    await this.setState({ [name]: { data: value } });
    await this.verificasPass()
    let valid = await this[name].validity.valid
  ...
  verificasPass() {
    ...
    if (this.state.password.data !== this.state.confpassword.data) {
      this.confpassword.setCustomValidity("La contraseñas no coinciden.");
    } else {
